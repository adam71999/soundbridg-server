...
Receives uploaded audio exports from the Mac agent and serves them via the web player.
"""
...
def health():
    meta = load_meta()
    total = sum((UPLOAD_FOLDER / f).stat().st_size for f in meta if (UPLOAD_FOLDER / f).exists())
    return jsonify({"status": "ok", "tracks": len(meta), "storage_mb": round(total / 1024 / 1024, 2)})
...
def upload():
    ...
    if dest.exists():
        ...
        while dest.exists():
            filename = f"{stem}_{n}{ext}"; dest = UPLOAD_FOLDER / filename; n += 1
